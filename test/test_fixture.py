...
async def body(p):
    ok_all = True
    for mode in ("classic", "modern", "unknown"):
        await p.cdp.send("Page.navigate", {"url": f"http://127.0.0.1:{PORT}/watch?mode={mode}"}, session=p.s)
        await p.wait_js("document.readyState==='complete' && !!document.getElementById('show')", 10)
        await p.ev("window.chrome={runtime:{onMessage:{addListener(){}}}}; true")
        await p.ev(SRC + "; true")
        ...
        for shift in (False, True):
            ...
            ok_all &= ok
            print(f"[{mode}{' shift' if shift else ''}] {'PASS' if ok else 'FAIL'} label={label!r} panels={panel_state} copied={copied!r}")
